...
export function renderStatusPanelLines(statusState, pulseFrame = 0) {
    ...
    lines.push('{gray-fg}Containers{/gray-fg}');
    lines.push(`  Active: ${statusState.containers.filter((container) => container.status.toLowerCase().startsWith('up')).length}`);
    lines.push('');

    if (statusState.containers.length === 0) {
        lines.push('  no running containers');
        return lines;
    }

    for (const container of statusState.containers) {
