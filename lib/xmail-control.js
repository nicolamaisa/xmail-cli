...
function buildDerivedEnv(values) {
  ...
  const hasTemplateDatabaseUrl =
    typeof derived.DATABASE_URL === "string" &&
    derived.DATABASE_URL.includes("${");

  if (!derived.DATABASE_URL || hasTemplateDatabaseUrl) {
    derived.DATABASE_URL = `postgres://${derived.POSTGRES_USER}:${derived.POSTGRES_PASSWORD}@x-db:5432/${derived.POSTGRES_DB}`;
  }
