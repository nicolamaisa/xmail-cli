...
async function postgresVolumeExists() {
  try {
    await execFileAsync(
      "docker",
      ["volume", "inspect", XMAIL_POSTGRES_VOLUME],
      {
        cwd: XMAIL_ROOT,
      },
    );
...
export async function resetDbData(ctx) {
  try {
    ...
    ctx.log(
      chalk.yellow(
        "DB-related services stop returned a non-zero status; continuing reset.",
      ),
    );
  }
  ...
  try {
    await runLoggedCommand(
      ...
      "docker",
      ["volume", "rm", "-f", XMAIL_POSTGRES_VOLUME],
      {
    ...
    ctx.log(
      chalk.yellow(
        `Volume ${XMAIL_POSTGRES_VOLUME} was not removed cleanly; continuing.`,
      ),
    );
  }
