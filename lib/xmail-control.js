...
const XMAIL_ROOT = '/opt/xmail-prod';
...
async function readDockerContainers() {
    try {
        ...
        return stdout
            ...
            .filter((container) => container.name.startsWith('xmail-') || container.name.startsWith('x-'));
    } catch {
        return [];
    }
}
...
async function postgresVolumeExists() {
    try {
        await execFileAsync('docker', ['volume', 'inspect', XMAIL_POSTGRES_VOLUME], {
            cwd: XMAIL_ROOT
        });
        return true;
    } catch {
...
function buildDerivedEnv(values) {
    const publicUrl = values.PUBLIC_URL || values.SITE_URL || 'http://localhost:8000';
...
export async function discoverXmailState() {
    const [exampleContent, envContent, containers, dbVolumeExists] = await Promise.all([
    ...
    const missingRequiredKeys = REQUIRED_ENV_KEYS.filter((key) => !mergedValues[key]);
    ...
    return {
        ...
        composeExists: (await fs.access(XMAIL_COMPOSE_FILE).then(() => true).catch(() => false)),
...
export async function generateJwtKeys() {
    const { stdout } = await execFileAsync('node', ['scripts/generate-keys.mjs'], {
        cwd: XMAIL_ROOT
    });
...
export function runLoggedCommand(ctx, title, command, args, options = {}) {
    return new Promise((resolve, reject) => {
        ...
        const flushStdout = wireProcessStream(child.stdout, (line) => ctx.logs.appendProcessLog(processId, line));
        const flushStderr = wireProcessStream(child.stderr, (line) => ctx.logs.appendProcessLog(processId, `{red-fg}${line}{/red-fg}`));

        child.on('error', (error) => {
            ctx.logs.finishProcessLog(processId, chalk.red(`Failed: ${error.message}`));
            reject(error);
        });
        ...
        child.on('close', (code) => {
            ...
            if (code === 0) {
                ctx.logs.finishProcessLog(processId, chalk.green(options.successFooter || 'Completed'));
                resolve();
                return;
            }

            const error = new Error(`${title} failed with exit code ${code}`);
            ctx.logs.finishProcessLog(processId, chalk.red(options.failureFooter || `Failed with exit code ${code}`));
            reject(error);
        });
    });
}
...
export async function resetDbData(ctx) {
    try {
        await runLoggedCommand(ctx, 'Stop DB-related services', 'docker', ['compose', 'stop', 'x-api', 'x-auth', 'x-rest', 'x-db']);
    } catch {
        ctx.log(chalk.yellow('DB-related services stop returned a non-zero status; continuing reset.'));
    }

    try {
        await runLoggedCommand(ctx, 'Remove Postgres volume', 'docker', ['volume', 'rm', '-f', XMAIL_POSTGRES_VOLUME], {
            successFooter: `Removed ${XMAIL_POSTGRES_VOLUME}`
        });
    } catch {
        ctx.log(chalk.yellow(`Volume ${XMAIL_POSTGRES_VOLUME} was not removed cleanly; continuing.`));
    }

    ctx.log(chalk.green(`DB volume reset: ${XMAIL_POSTGRES_VOLUME}`));
}
...
export async function bootstrapXmailStack(ctx, options) {
    ...
    await runLoggedCommand(ctx, 'Start core services', 'docker', ['compose', 'up', '-d', 'x-db', 'x-auth'], {
    ...
    await runLoggedCommand(ctx, 'Start remaining services', 'docker', composeArgs, {
        successFooter: options.devMode ? 'Stack started with build' : 'Stack started'
    });

    if (options.showStatusAfter) {
