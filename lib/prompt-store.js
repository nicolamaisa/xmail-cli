...
export function createPromptStore(logs, options = {}) {
  /** @type {{ title: string, mode: 'single' | 'history', entries: Array<PromptEntry & { optionCursor?: number, validationError?: string }>, activeIndex: number, resolve: (value: PromptResult | null) => void } | null} */
  let activeForm = null;
  ...
  /**
   * @param {PromptEntry & { optionCursor?: number, validationError?: string }} entry
  ...
  function renderQuestion(entry, isActive) {
    ...
    if (entry.type === "select") {
      return [
        ...
        ...entry.options.map((option, index) => {
          ...
          const line = `{gray-fg}│{/gray-fg}  ${mark} ${option.label}`;
    ...
    if (entry.validationError) {
      lines.push(`  {red-fg}${entry.validationError}{/red-fg}`);
    }
  ...
  function renderForm() {
    ...
    const instructionsLine =
      "{gray-fg}{bold}Left/Right{/bold} change  {bold}Space{/bold} toggle  {bold}Enter{/bold} continue  {bold}Esc{/bold} cancel{/gray-fg}";
    options.onInstructionsChange?.(instructionsLine);
    ...
    handleKeypress(ch, key = {}) {
      ...
      if (key.name === "return" || key.name === "enter") {
        ...
        if (
          ...
          entry.validationError = "This field is required.";
          renderForm();
          return true;
        }
