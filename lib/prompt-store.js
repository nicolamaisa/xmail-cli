...
export function createPromptStore(logs, options = {}) {
    ...
    function renderForm() {
        ...
        const totalQuestions = currentForm.entries.filter((entry) => entry.type !== 'info').length;
    ...
    function nextInteractiveIndex(startIndex) {
        ...
        while (index < activeForm.entries.length && activeForm.entries[index]?.type === 'info') {
            index += 1;
        }
        return index;
    }
    ...
    function changeChoice(entry, direction) {
        ...
        const currentIndex = typeof entry.optionCursor === 'number'
            ? entry.optionCursor
            : Math.max(0, entry.options.findIndex((option) => option.value === entry.value));
        const nextIndex = Math.max(0, Math.min(entry.options.length - 1, currentIndex + direction));
        entry.optionCursor = nextIndex;

        if (entry.type === 'select') {
            entry.value = entry.options[nextIndex]?.value || entry.options[0]?.value || '';
        }
    }
    ...
    return {
            ...
            const entries = definition.entries.map((entry) => {
                if (entry.type === 'select') {
                    ...
                    return {
                        ...entry,
                        value: initialValue,
                        optionCursor: Math.max(0, entry.options.findIndex((option) => option.value === initialValue))
                    };
                }
        ...
        handleKeypress(ch, key = {}) {
            ...
            if (key.name === 'backspace' && (entry.type === 'text' || entry.type === 'password')) {
            ...
            if ((entry.type === 'text' || entry.type === 'password') && ch && ch >= ' ') {
