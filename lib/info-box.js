 ...
 * @param {string} title
 * @param {string} content
 * @param {{ pointer?: string, titleColor?: string, borderColor?: string, maxWidth?: number, tone?: 'info' | 'warning' | 'error' | 'success' }} [options]
 * @returns {string[]}
 */
export function renderInfoBox(title, content, options = {}) {
  ...
  const toneColors =
    tone === "error"
      ? { titleColor: "red-fg", borderColor: "red-fg" }
      : tone === "warning"
        ? { titleColor: "yellow-fg", borderColor: "yellow-fg" }
        : tone === "success"
          ? { titleColor: "green-fg", borderColor: "green-fg" }
          : { titleColor: "magenta-fg", borderColor: "gray-fg" };
  const titleColor = options.titleColor ?? toneColors.titleColor;
  const borderColor = options.borderColor ?? toneColors.borderColor;
