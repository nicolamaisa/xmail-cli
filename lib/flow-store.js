...
export function createFlowStore(logs, options = {}) {
  ...
  function renderProcess(entry) {
    ...
    const titleSuffix =
      elapsedValue === null
        ? ""
        : ` {gray-fg}(${formatElapsed(elapsedValue)}){/gray-fg}`;
  ...
  function renderEntry(entry) {
    ...
    if (entry.type === "answer") {
      return [
        `{gray-fg}♢{/gray-fg} {magenta-fg}${entry.label}{/magenta-fg}`,
  ...
  function renderQuestion(entry) {
    if (entry.type === "text" || entry.type === "password") {
      ...
      const lines = [
        `{cyan-fg}♦{/cyan-fg} {magenta-fg}{bold}${entry.label}{/bold}{/magenta-fg}`,
    ...
    return [
      ...
      ...entry.options.map((/** @type {PromptChoice} */ option, /** @type {number} */ index) => {
        ...
        const rendered = index === entry.optionCursor ? `{cyan-fg}{bold}${line}{/bold}{/cyan-fg}` : selected ? `{green-fg}${line}{/green-fg}` : `{gray-fg}${line}{/gray-fg}`;
  ...
  return {
    ...
    addInfo(title, content, infoOptions = {}) {
      if (!activeFlow) {
        return;
      }
    ...
    appendProcess(id, line) {
      ...
      const wrapped = wrapLine(line, Math.max(20, logs.getPromptWidth() - 8));
